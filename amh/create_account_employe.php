...
<h2>Create Account Form FOR EMPLOYEE'S</h2>
<br>
<br>
...
<form method="post" action="#" >
	...
	<input type="number" name="security" required="" id="secno" placeholder="enter security number" class="form-control">
...
if ($password == $confirmpassword) {
	...
	$query = "SELECT * FROM employe WHERE securityno= '$securitynumber'";
  $user_count = mysqli_query($con,$query);
  if(mysqli_num_rows($user_count) > 0) {
$chekfornull= "SELECT username FROM employe WHERE securityno= '$securitynumber'";
...
if (empty($username)) {
	...
	$insert="UPDATE employe SET username='$usernamee' , password='$password' WHERE securityno='$securitynumber' ";
  ...
  window.location='login.php';
   ...
   ";
  		...
  		echo "<script>alert('not registered');</script>";
  					 echo "
    <script>
  window.location='create_account_employe.php';
 </script>
   ";
  	}
}else{
