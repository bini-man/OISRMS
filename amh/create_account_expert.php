...
<h2>Create Account Form FOR EXPERT'S</h2>
<br>
<br>
...
<form method="post" action="#" >
	...
	<input type="number" name="security" required="" id="secno" class="form-control" placeholder="enter security number">
...
if ($password == $confirmpassword) {
	...
	$query = "SELECT * FROM expert WHERE securityno= '$securitynumber'";
  $user_count = mysqli_query($con,$query);
  if(mysqli_num_rows($user_count) > 0) {
$chekfornull= "SELECT username FROM expert WHERE securityno= '$securitynumber'";
...
if (empty($username)) {
	...
	$insert="UPDATE expert SET username='$usernamee' , password='$password' WHERE securityno='$securitynumber' ";
  ...
  window.location='login.php';
   ...
   ";
  		...
  		echo "<script>alert('not registered');</script>";
  			 echo "
    <script>
  window.location='create_account_expert.php';
 </script>
   ";
  	}
}else{
